...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        self.embed_to_hidden = nn.Linear(self.n_features * self.embed_size, self.hidden_size) #input to hidden
        nn.init.xavier_uniform_(self.embed_to_hidden.weight) #check documentation
        self.dropout = nn.Dropout(self.dropout_prob)
        self.hidden_to_logits = nn.Linear(self.hidden_size,  self.n_classes) #hidden to output to logistic
        nn.init.xavier_uniform_(self.hidden_to_logits.weight) #check
        ### END CODE HERE
    ...
    def embedding_lookup(self, t):
        ...
        temptensor = self.pretrained_embeddings(t)
        x = temptensor.view(-1, self.n_features * self.embed_size)
        ### END CODE HERE
        return x
    ...
    def forward(self, t):
        ...
        one = self.embedding_lookup(t)
        two = self.embed_to_hidden(one)
        three = nn.functional.relu(two)
        four = self.dropout(three)
        logits = self.hidden_to_logits(four)
