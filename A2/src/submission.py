...
def naive_softmax_loss_and_gradient(center_word_vec,outside_word_idx,outside_vectors,dataset):
  ...
  ### START CODE HERE
  predicted_dist = softmax(np.dot(outside_vectors, center_word_vec)) #y hat
  true_dist = np.zeros_like(predicted_dist) #y
  true_dist[outside_word_idx] = 1
   
  
  loss = -np.log(predicted_dist[outside_word_idx]) #compute loss specifically on outside word idx
  
  grad_center_vec = np.dot((predicted_dist - true_dist), outside_vectors) #U(^y - y)
  
  grad_outside_vecs = np.outer((predicted_dist - true_dist), center_word_vec) #vc(^y - y)
  ### END CODE HERE
...
def skipgram(current_center_word, window_size, outside_words, word2ind, center_word_vectors, outside_vectors, dataset, word2vec_loss_and_gradient=neg_sampling_loss_and_gradient):
  ...
  for word in outside_words:
    ...
    loss_current, gradc, grado = word2vec_loss_and_gradient(center_word_vec,outsideWordIdx,outside_vectors, dataset)
    loss += loss_current
