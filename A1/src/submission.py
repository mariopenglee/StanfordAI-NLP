...
def distinct_words(corpus):
    ...
    # ### START CODE HERE ###
    flat_list = [x for xs in corpus for x in xs]
    [corpus_words.append(x) for x in flat_list if x not in corpus_words]
...
def compute_co_occurrence_matrix(corpus, window_size=4):
    ...
    for document in corpus:
        for i, centerword in enumerate(document):
            if i-window_size<0:
                contextwords = document[0:i]
            else:
                contextwords = document[i-window_size:i]
            if i+window_size>len(document):
                contextwords += document[i+1:len(document)]
            else:
                contextwords += document[i+1:i+window_size+1]
            centerwordIND = word2Ind[centerword]
            for contextword in contextwords:
                M[centerwordIND][word2Ind[contextword]] += 1
